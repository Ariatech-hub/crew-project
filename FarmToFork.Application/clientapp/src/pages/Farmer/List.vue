  ...
  <q-page>
    <q-card flat class="no-border-radius">
      ...
      <q-card-section style="padding-top: 0px">
        <div>
          <div class="row justify-center bg-white q-py-md q-mb-md bordered">
            <div class="col-md-4 col-xs-12 q-pl-md">
              <q-select
                dense
                outlined
            ...
            <div class="col-md-4 col-xs-12 q-pr-md">
              <q-select
                dense
                outlined
          ...
          >
            ...
            <template v-slot:body="props">
              <tr :key="props.row.id">
                ...
                <td class="text-left">{{ props.row.tole }}</td>
                ...
                <td class="text-center">
                  <q-toggle
                    v-model="props.row.isNonMember"
                    size="xs"
                    dense
                    color="primary"
                    @update:model-value="
                      onIsNonMemberToggleClicked(props.row.id)
                    "
                  />
                </td>
                <td class="text-right">
...
export default defineComponent({
  setup() {
    ...
    const onIsNonMemberToggleClicked = async (farmerId) => {
      try {
        $q.loading.show();
        const response = await api.post(`farmer/change-non-member-status`, {
          Id: farmerId,
        });
    ...
    //     $q.dialog({
    //       title: "Confirm",
    //       message: `Are you sure you want to delete this market ${market.name}?`,
    //       cancel: true,
    //       persistent: true,
    //     }).onOk(async () => {
    //       $q.loading.show();
    //       try {
    //         let response = await api.post("setting/market/delete", {
    //           id: market.id,
    //         });
    //         $q.notify({
    //           type: "positive",
